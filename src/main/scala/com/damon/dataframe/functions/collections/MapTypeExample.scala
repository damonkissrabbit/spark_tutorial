...
import org.apache.spark.sql.functions.{map_concat, map_keys, map_values}
...
object MapTypeExample {
  def main(args: Array[String]): Unit = {
    ...
    mapTypeDF.select(
      $"name",
      map_keys($"properties").alias("map_keys"),
      map_values($"properties").alias("map_values"),
      map_concat($"properties", $"secondProp").alias("map_concat")
    ).show(false)
